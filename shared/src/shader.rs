...
impl Shader {
    ...
    pub fn new(
        ...
        unsafe {
            ...
            match g_shader_code {
                Some(g_shader_code) => {
                    geometry = Some(gl::CreateShader(gl::GEOMETRY_SHADER));
                    gl::ShaderSource(geometry.unwrap(), 1, &g_shader_code.as_ptr(), ptr::null());
                    gl::CompileShader(geometry.unwrap());
                    Self::check_compile_errors(geometry.unwrap(), "GEOMETRY");
                }
                None => {
                    geometry = None;
                }
