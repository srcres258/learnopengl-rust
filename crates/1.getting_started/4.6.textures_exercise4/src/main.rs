...
fn main() {
    ...
    unsafe {
        ...
        while !window.should_close() {
            ...
            our_shader.set_float("mixValue".to_string(), MIX_VALUE);
...
fn process_input(
    ...
    match event {
        ...
        WindowEvent::Key(Key::Up, _, Action::Repeat, _) => {
            unsafe {
                MIX_VALUE += 0.001; // change this value accordingly (might be too slow or too fast based on system hardware)
                if MIX_VALUE >= 1.0 {
                    MIX_VALUE = 1.0;
                }
            }
