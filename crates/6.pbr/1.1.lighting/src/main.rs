...
fn main() {
    ...
    unsafe {
        ...
        // -------------------------
        let shader = Shader::new("1.1.pbr.vs".to_string(), "1.1.pbr.fs".to_string(), None);

        shader.use_shader();
        shader.set_vec3_coords("albedo".to_string(), 0.5, 0.0, 0.0);
        shader.set_float("ao".to_string(), 1.0);
        ...
        while !window.should_close() {
            ...
            for row in 0..nr_rows {
                ...
                for col in 0..nr_columns {
                    ...
                    // on direct lighting.
                    shader.set_float("roughness".to_string(), util::glm::clamp(col as f32 / nr_columns as f32, 0.05, 1.0));
