...
lazy_static! {
    // lighting
    static ref LIGHT_POS: Mutex<glm::TVec3<f32>> = Mutex::new(glm::vec3(1.2, 1.0, 2.0));
}
...
fn main() {
    ...
    unsafe {
        ...
        // ------------------------------------
        let lighting_shader = Shader::new("2.2.basic_lighting.vs".to_string(), "2.2.basic_lighting.fs".to_string());
        let lighting_cube_shader = Shader::new("2.2.light_cube.vs".to_string(), "2.2.light_cube.fs".to_string());
        ...
        while !window.should_close() {
            ...
            LIGHT_POS.lock().unwrap().x = 1.0 + glfw.get_time().sin() as f32 * 2.0;
            LIGHT_POS.lock().unwrap().y = (glfw.get_time() / 2.0).sin() as f32;

            // be sure to activate shader when setting uniforms/drawing objects
            lighting_shader.use_shader();
            lighting_shader.set_vec3_coords("objectColor".to_string(), 1.0, 0.5, 0.31);
            lighting_shader.set_vec3_coords("lightColor".to_string(), 1.0, 1.0, 1.0);
            lighting_shader.set_vec3("lightPos".to_string(), &LIGHT_POS.lock().unwrap());
            ...
            model = glm::translate(&model, &LIGHT_POS.lock().unwrap());
